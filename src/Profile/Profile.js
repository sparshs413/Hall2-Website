	...
	authListener() {
		...
		Firebase.firestore()
			...
			.then((querySnapshot) => {
				...
				this.setState({ docs: matches, docsItems: items });
			})
			.catch(function (error) {
				console.log('Error getting documents: ', error);
			});
		...
		Firebase.firestore()
			...
			.then((querySnapshot) => {
				...
				this.setState({ alumniDocs: matches, alumniID: items });
			})
			.catch(function (error) {
				console.log('Error getting documents: ', error);
			});
				...
				<Container text className='edit_profile_container' style={{ marginTop: '1em' }}>
					{this.state.isLoading ? (
						...
						<>
							...
							<Button type='submit' color='linkedin' className='alumni_form_button' disabled={this.state.uploading} onClick={this.changeUserData}>
							...
							<Form>
								<img
									...
									alt=''
									src={this.state.userImage ? this.state.userImage : require('../Alumni/stu.jpeg')}
								...
								<div className='custom-file profile_image'>
									...
									<span style={{ color: 'indigo' }}>
										Crop image in square shape before uploading for good cover <br /> After uploading the Profile Photo, dont click on the submit button untill the photo is being
