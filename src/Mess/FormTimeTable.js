...
class FormTimeTable extends Component {
	...
	componentDidMount() {
		...
		Firebase.firestore()
			...
			.then((querySnapshot) => {
				...
				arr.push(object);
				console.log(arr[0].sunday.breakfast[0]);
	...
	createUI(day = "sunday", time = 0) {
		...
		return v.map((el, i) => (
			<div key={i}>
				<Input
					action={<input type="button" value="x" className="remove" onClick={this.removeClick.bind(this, day, time, i)} />}
					iconPosition="left"
					// name=
					value={el || ""}
									...
									<tr>
										<td
											style={{
												...
												fontWeight: `${this.state.current_day === i ? "bold" : ""}`,
											}}
										>
											{day.toString()}
										</td>
