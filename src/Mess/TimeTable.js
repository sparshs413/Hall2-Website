...
import { Tab, Container, Card, Button, Item, Label, Icon, Transition } from 'semantic-ui-react'
import "./Extras.css";
...
class TimeTable extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      current_day: 0,
      visible: false,
      values: [{sunday:['aa','c'], monday:['q','w']},
               {sunday:['aa2','c2'], monday:['q2','w2']},
               {sunday:['aa3','c3'], monday:['q3','w3']}],
      extras: [{sunday:{'aa':[3,7],'c':[33,73]}, monday:{'d':[3,7],'e':[3,7]}},
              {sunday:{'ala':[31,71],'sc':[13,17]}, monday:{'ffd':[3,7],'dee':[3,7]}},
              {sunday:{'ada':[32,72],'c1':[23,27]}, monday:{'x':[3,2],'cx':[4,2]}}]
            ...
            <Item>
              <Item.Content>
                {values[i][day].map((item) => { 
                  return (
                  <><Item.Header style={{margin: '2px 0'}}>{item}</Item.Header><br/></>
                  )}
                )}
              </Item.Content>
            </Item>
            ...
            <Item style={{padding: '12px 0'}}>
              ...
              return (
                <>
                  ...
                  <Label style={{float: 'right'}}>₹ {extras[i][day][key][0]}</Label>
                  <Label>{extras[i][day][key][1]} left</Label><br/>
                </>
                )}
              )}
              </Item.Content>
