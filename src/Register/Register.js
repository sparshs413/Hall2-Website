...
class Register extends Component {
	...
	onSubmit = async (e) => {
		...
		if (password1 !== password2) {
			...
			await db.collection('users-data').add({
				...
				<Grid textAlign='center' style={{ height: '80vh' }} verticalAlign='middle'>
					<Grid.Column style={{ maxWidth: 500, margin: 10 }}>
						...
						<Form size='large' onSubmit={this.onSubmit}>
							<Segment stacked>
								<Form.Input fluid name='name' icon='user' iconPosition='left' placeholder='Name' required onChange={this.onChange} value={this.state.name} />
								<Form.Input
									fluid
									name='email'
									type='email'
